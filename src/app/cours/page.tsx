...
const questions = [
  {
    id: 1,
    question: "Quel est la couleur du cheval blanc de Sherzod ?",
  ...
  },
  {
    ...
    options: [".ma-classe", "#mon-id", "div", "*"],
    correctIndex: 0,
  },
  {
    id: 3,
    question: "Quelle propriété change la couleur du texte ?",
    options: ["background", "color", "font-size", "border"],
    correctIndex: 1,
  },
  {
    id: 4,
    question: "Quelle propriété gère l'espace intérieur (padding) ?",
    options: ["margin", "padding", "gap", "size"],
    correctIndex: 1,
  },
];
  ...
  return (
    <div className="bg-[#1D1D1D] min-h-screen font-[silkscreen]">
      ...
      <div className="grid grid-cols-2">
        <div className="flex items-center justify-start flex-col p-16">
          <h2 className="w-full text-center text-5xl text-[#1985a7]">Cours</h2>

          {courseSections[page] ? (
            <p className="mt-4 text-white mb-10">
              {courseSections[page].content}
            </p>
          ) : (
            <LoaderElement loader={true} h="h-[200px]" />
          )}
        </div>
        ...
        <div className={"grid grid-rows-2 p-4 gap-4"}>
          <div className="h-[80%]">
            {course && (
              <label className="text-2xl text-[#1985a7] text-center mb-2 block">
                Éditeur {course.idLanguage}
              </label>
            )}
            <textarea
              value={code}
          ...
          {language?.idLanguage !== 7 && (
            <div className="bg-white rounded overflow-hidden">
              {course && (
                <label className="text-2xl block text-[#1985a7] text-center pr-2">
                  Aperçu ({course.idLanguage} rendu)
                </label>
              )}
              <iframe
                ref={iframeRef}
      ...
      <div className="w-full text-center mb-10 gap-5 flex flex-row items-center justify-center">
        {page !== 0 && (
          <button
            ...
            className="w-1/4 text-2xl outline-[#989AAF] outline-2 border-2 cursor-pointer border-[#FFFFFF] rounded bg-[#DADCE7] shadow-[0px_2px_0px_2px_#666880] hover:shadow-none hover:mt-0.5"
          >
            Précédent
          </button>
        )}
