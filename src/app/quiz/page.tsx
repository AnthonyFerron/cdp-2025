...
import { IdCourse, IdQuiz, IdUser } from "../types/custom.types";
...
export default function QuizPage() {
  ...
  const loadCourseLanguage = async () => {
    const idCourse = new URL(document.URL).searchParams.get("idCourse");

    if (idCourse) {
      const course = await getCourse(parseInt(idCourse) as IdCourse);
      if (course) {
        // Mapper l'idLanguage vers le nom du langage pour l'URL
        const languageMap: { [key: number]: string } = {
          6: "html",
          7: "python",
          8: "css",
        };
        const language = languageMap[course.idLanguage] || "";
  ...
  const getResponse = async () => {
    if (quizQuestions[page]) {
      const res = await getQuizQuestion(
        quizQuestions[page].idQuizQuestion,
        true
      );
      if (res) {
        setResponse(res);
        ...
        {quizQuestions.length === page && quizQuestions.length !== 0 ? (
          <div className="h-[400px] flex flex-col items-center justify-around">
            {(correctAnswer / quizQuestions.length) * 100 >= 75 ? (
              <p className="text-green-500 font-[silkscreen] text-2xl">
                Vous avez réussis le cours
              </p>
            ) : (
              <p className="text-red-500 font-[silkscreen] text-2xl">
                Vous avez échoué le cours
              </p>
            )}

            <p className="text-white font-[silkscreen] text-8xl">
              {correctAnswer}
            </p>
            ...
            {languageLoading ? (
              ...
              <button
                onClick={() => {
                  const redirectUrl = courseLanguage
                    ? `/carte/${courseLanguage}`
                    : "/accueil";
              ...
              page < quizQuestions.length && (
                <div className="grid grid-cols-2 gap-5 my-4 h-[175px]">
                  {quizQuestions[page].choices.map((opt, i) => {
                    ...
                    let buttonColor =
                      "bg-[#DADCE7] border-[#FFFFFF] text-black";

                    if (response) {
                      if (response.answer.includes(i)) {
                        buttonColor =
                          "bg-green-500 border-green-900 text-white";
                      } else if (
                        choices.includes(i) &&
                        !response.answer.includes(i)
                      ) {
